// Shader Created by Lallis in 2015-05-15
...
void main()
{
	...
	vec2 p = uv*2.0-1.0;
	p.x*=(resolution.x/resolution.y)/scale;
	p.y*=(resolution.x/resolution.y)/scale;
	vec3 col = vec3(0.0);
	vec3 ro = vec3(0.0,0.0,-2.0);
	vec3 rd = normalize(vec3(p,0.0)-ro);
	ro*=rot(vec3(time*0.25f-2.0*PI/resolution.y,time*0.25f+2.0*PI/resolution.x,0.0));
	rd*=rot(vec3(time*0.25f-2.0*PI/resolution.y,time*0.25f+2.0*PI/resolution.x,0.0));
	vec3 rp = ro;
	if(pow(dot(rd,ro-vec3(0.0)),2.0) - pow(length(ro-vec3(0.0)),2.0) + 1.0>0.0)
	{
		...
		for(int i = 0; i<64; i++)
		{
			...
			if(ds>0.0) break;
			col = mix(col,
			vec3(1.0*abs(sin(time*0.25f)),1.5*abs(ds),1.0*abs(sin(time*0.25f+PI*0.5))),
